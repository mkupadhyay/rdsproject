...
/**
 * Resolver for Mutation.unsubscribeFromLeads field
 */
...
module.exports.handler = async event => {
  ...
  // 4. Get Subscription Arns to unsubscribe
  ...
  try {
    ...
    console.log('Successfully, got subscriptionArns');
  } catch (error) {
    console.error(JSON.stringify(error));
    throw error;
  }

  // 5. Unsubscribe subscriptions
  promises = [];
  ...
  try {
    ...
    console.log('Successfully, unsubscribed all subscriptions');
  } catch (error) {
    console.error(JSON.stringify(error));
    throw error;
  }

  // 6. Return subscription ARN list unsubscribed
